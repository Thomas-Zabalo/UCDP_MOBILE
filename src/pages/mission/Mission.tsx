...
// Mock data pour simuler les pros qui ont accepté
const MOCK_PROS = [
        ...
        <div className="min-h-screen bg-white">
            ...
            {selectedMission === "1" && (
                <div className="fixed inset-0 z-[100] flex items-end">
                    ...
                    {/* Contenu du Panel */}
                    <div className="relative bg-white w-full rounded-t-[40px] px-8 pt-4 pb-12 animate-in slide-in-from-bottom-full duration-500 shadow-[0_-20px_50px_-12px_rgba(0,0,0,0.3)]">
                        {/* Barre de drag visuelle */}
                        <div className="w-12 h-1.5 bg-gray-200 rounded-full mx-auto mb-8" />

                        <div className="flex justify-between items-start mb-8">
                            <div>
                                <h3 className="text-2xl font-black uppercase tracking-tighter text-black">Candidatures</h3>
                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">Sélectionnez votre artisan</p>
                            </div>
                            <button
                                onClick={() => setSelectedMission(null)}
...
function MissionItem({ title, status, date, isActive, onClick }: any) {
    return (
        <button
            onClick={onClick}
            className={`w-full p-6 rounded-[32px] border-2 text-left transition-all active:scale-[0.98] ${isActive ? 'border-black bg-black text-white shadow-2xl shadow-black/20' : 'border-gray-100 bg-gray-50 text-black'}`}
        >
            <div className="flex justify-between items-start mb-4">
                <div className={`size-12 rounded-2xl flex items-center justify-center ${isActive ? 'bg-white/10 border border-white/20' : 'bg-white shadow-sm'}`}>
                    <IonIcon name="construct" className={`text-xl ${isActive ? 'text-white' : 'text-black'}`} />
                </div>
                <span className={`text-[10px] font-black uppercase tracking-widest ${isActive ? 'text-white/40' : 'text-gray-300'}`}>{date}</span>
            </div>
            <h2 className="text-xl font-black uppercase tracking-tighter mb-1 truncate">{title}</h2>
            <div className="flex items-center gap-2">
                <div className={`size-1.5 rounded-full ${isActive ? 'bg-green-400' : 'bg-black'} animate-pulse`} />
...
function ProCard({ pro }: any) {
    return (
        <div className="bg-white border border-gray-100 p-4 rounded-[30px] shadow-sm flex flex-col gap-4">
            <div className="flex items-center gap-4">
                <div className="size-16 shrink-0 rounded-[22px] overflow-hidden border border-gray-50 shadow-inner">
                    <img src={pro.img} className="w-full h-full object-cover" alt={pro.name} />
                </div>

                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                        <h4 className="font-black text-sm uppercase tracking-tight text-black truncate">{pro.name}</h4>
                    ...
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2">{pro.job}</p>
                    <span className="text-[11px] font-black text-black italic">Proposition : {pro.price}</span>
                </div>
            </div>

            <div className="flex gap-2">
                <button className="flex-1 h-12 bg-black text-white rounded-2xl font-black uppercase text-[10px] tracking-widest active:scale-95 transition-all shadow-lg shadow-black/10">
                    Accepter
                </button>
                <button className="size-12 bg-gray-100 text-black rounded-2xl flex items-center justify-center active:scale-95 transition-all">
